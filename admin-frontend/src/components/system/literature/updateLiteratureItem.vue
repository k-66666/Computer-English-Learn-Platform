<template>
<div>
  <el-dialog title="编辑文献章节" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  import {editApeLiteratureItem,getApeLiteratureItemById} from '../../../api/api'
  export default {
    data() {
      return{
        editorOption: {
    ...
    props: ['updateVisible','updateId','literatureId'],
    methods: {
      submit() {
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            this.form.literatureId = this.literatureId
            editApeLiteratureItem(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify.success({
                  title: '成功',
                  message: "保存成功"
    ...
    watch: {
      updateId(newVal) {
        if(newVal) {
          getApeLiteratureItemById({id:newVal}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
