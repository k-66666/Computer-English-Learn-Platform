<template>
<div>
  <el-dialog title="编辑自测" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  ...
  import {editApeExamine,getApeExamineById} from '../../../api/api'
  export default {
    mixins: [mixin],
    data() {
      return{
    ...
    methods: {
      ...
      submit() {
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            editApeExamine(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify.success({
                  title: '成功',
                  message: "保存成功"
    ...
    watch: {
      updateId(newVal) {
        if(newVal) {
          getApeExamineById({id:newVal}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
