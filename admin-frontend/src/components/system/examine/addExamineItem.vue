<template>
<div>
  <el-dialog title="新增题目" width="40%" :destroy-on-close="true" :visible.sync="addVisible" :before-close="handleClose">
...
<script>
  import {saveApeExamineItem} from '../../../api/api'
  export default {
    data() {
      return{
        formLabelWidth: '80px',
      ...
      submit() {
        ...
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            ...
            saveApeExamineItem(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify.success({
                  title: '成功',
                  message: "保存成功"
