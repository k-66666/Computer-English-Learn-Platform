        ...
        <div class="table">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button size="mini" type="success" @click="toInfo(scope.row.id)">评论</el-button>
                <el-popconfirm
...
<script>
  import {getApeDiscussPage,removeApeDiscuss} from '../../../api/api' 
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      toInfo(id) {
        var param = {
            "name": "讨论评论",
            "url": "/discussItem?id=" + id 
        }
        this.$store.commit('menu/addActiveMenu', param)
        this.$router.push("/discussItem?id=" + id)
        this.$store.commit('menu/setActiveMenu', "/discussItem?id=" + id)
      },
      searchPage() {
      ...
      query() {
        getApeDiscussPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeDiscuss({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
