        ...
        <div class="table">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="toItem(scope.row.id)">章节</el-button>
...
<script>
  import {getApeLiteraturePage,removeApeLiterature} from '../../../api/api' 
  import add from '../../../components/system/literature/addLiterature'
  import update from '../../../components/system/literature/updateLiterature'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      toItem(id) {
        var param = {
            "name": "文献章节",
            "url": "/literatureItem?id=" + id 
        }
        this.$store.commit('menu/addActiveMenu', param)
        this.$router.push("/literatureItem?id=" + id)
        this.$store.commit('menu/setActiveMenu', "/literatureItem?id=" + id)
      },
      query() {
        getApeLiteraturePage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeLiterature({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
