...
<script>
  import {getApeSyntaxTypePage,removeApeSyntaxType} from '../../../api/api' 
  import add from '../../../components/system/syntax/addSyntaxType'
  import update from '../../../components/system/syntax/updateSyntaxType'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeSyntaxTypePage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeSyntaxType({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
