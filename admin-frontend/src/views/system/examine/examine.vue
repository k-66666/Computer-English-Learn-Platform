        ...
        <div class="search">
            <el-row :gutter="10" style="padding:10px">
                <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
                    <span class="search-title">题目名称:</span>
                    <el-input
                ...
                <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
                    ...
                    <el-input
                        ...
                        size="mini"
                        placeholder="请输入简介"
                        v-model="search.introduction">
                    </el-input>
                </el-col>
                <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
                    <span class="search-title">题型:</span>
        ...
        <div class="table">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                  <el-tag v-if="scope.row.type == 0">代码选择</el-tag>
                  <el-tag v-if="scope.row.type == 1">职场听力</el-tag>
                  <el-tag v-if="scope.row.type == 2">代码判断</el-tag>
              </template>
            </el-table-column>
            <el-table-column
              prop="createTime"
            ...
            <el-table-column
              ...
              >
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="toItem(scope.row.id)">题目</el-button>
...
<script>
  import {getApeExaminePage,removeApeExamine} from '../../../api/api' 
  import add from '../../../components/system/examine/addExamine'
  import update from '../../../components/system/examine/updateExamine'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      toItem(id) {
        var param = {
            "name": "自测题目",
            "url": "/examineItem?id=" + id 
        }
        this.$store.commit('menu/addActiveMenu', param)
        this.$router.push("/examineItem?id=" + id)
        this.$store.commit('menu/setActiveMenu', "/examineItem?id=" + id)
      },
      searchPage() {
      ...
      query() {
        getApeExaminePage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeExamine({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
