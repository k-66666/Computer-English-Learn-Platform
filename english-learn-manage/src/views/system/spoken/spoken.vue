...
<script>
  import {getApeSpokenPage,removeApeSpoken} from '../../../api/api' 
  import add from '../../../components/system/spoken/addSpoken'
  import update from '../../../components/system/spoken/updateSpoken'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeSpokenPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeSpoken({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
