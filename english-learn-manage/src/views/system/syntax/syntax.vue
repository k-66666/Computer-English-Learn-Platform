...
<script>
  import {getApeSyntaxPage,removeApeSyntax} from '../../../api/api' 
  import add from '../../../components/system/syntax/addSyntax'
  import update from '../../../components/system/syntax/updateSyntax'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeSyntaxPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeSyntax({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
