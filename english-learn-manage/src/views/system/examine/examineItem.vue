<template>
  <div class="_item">
      ...
      <add @addFalse="addFalse" :examineId="search.examineId" :addVisible = "addVisible"></add>
      <update @updateFalse="updateFalse" :examineId="search.examineId" :updateId = "updateId" :updateVisible = "updateVisible"></update>
    </div>
  </div>
</template>

<script>
  import {getApeExamineItemPage,removeApeExamineItem} from '../../../api/api' 
  import add from '../../../components/system/examine/addExamineItem'
  import update from '../../../components/system/examine/updateExamineItem'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeExamineItemPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeExamineItem({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    mounted() {
      this.search.examineId = this.$route.query.id
      this.query()
    }
 }
</script>
