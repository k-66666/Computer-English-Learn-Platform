<template>
  <div class="_item">
      ...
      <add @addFalse="addFalse" :literatureId="search.literatureId" :addVisible = "addVisible"></add>
      <update @updateFalse="updateFalse" :literatureId="search.literatureId" :updateId = "updateId" :updateVisible = "updateVisible"></update>
    </div>
  </div>
</template>

<script>
  import {getApeLiteratureItemPage,removeApeLiteratureItem} from '../../../api/api' 
  import add from '../../../components/system/literature/addLiteratureItem'
  import update from '../../../components/system/literature/updateLiteratureItem'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeLiteratureItemPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeLiteratureItem({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    mounted() {
      var literatureId = this.$route.query.id
      if (literatureId) {
          this.search.literatureId = literatureId
      }
      this.query()
    }
 }
