...
<script>
  import {getApeDiscussItemPage,removeApeDiscussItem} from '../../../api/api' 
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeDiscussItemPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeDiscussItem({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    mounted() {
      this.search.discussId = this.$route.query.id
      this.query()
    }
 }
</script>
