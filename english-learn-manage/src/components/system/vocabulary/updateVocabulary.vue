<template>
<div>
  <el-dialog title="编辑词汇" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  ...
  import {editApeVocabulary,getApeVocabularyById} from '../../../api/api'
  export default {
    mixins: [mixin],
    data() {
      return{
      ...
      submit() {
        ...
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            ...
            editApeVocabulary(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify.success({
                  title: '成功',
                  message: "保存成功"
    ...
    watch: {
      updateId(newVal) {
        if(newVal) {
          getApeVocabularyById({id:newVal}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
              this.exampleList = JSON.parse(this.form.example)
