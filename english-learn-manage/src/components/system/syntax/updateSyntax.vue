<template>
<div>
  <el-dialog title="编辑语法" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  import {editApeSyntax,getApeSyntaxById,getApeSyntaxTypeList} from '../../../api/api'
  export default {
    data() {
      return{
        formLabelWidth: '80px',
        ...
        rules: {
          ...
          analysis: [
            { required: true, message: '请输入重要词汇', trigger: 'blur' },
          ],
        },
      }
    },
      ...
      submit() {
        ...
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            ...
            editApeSyntax(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify.success({
                  title: '成功',
                  message: "保存成功"
    ...
    watch: {
      updateId(newVal) {
        if(newVal) {
          getApeSyntaxById({id:newVal}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
              this.fenxi = JSON.parse(this.form.analysis)
            } else {
              this.$notify.error({
                title: '错误',
                message: res.message
